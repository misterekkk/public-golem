...
def build_histograms(X_binned, y, indices, features_indices, n_bins=256):

    n_features = len(features_indices)

    hist = np.zeros((n_features, n_bins, 2))
...
def build_tree(
    ...
    root_hist = build_histograms(X_binned, y, root_indices, features_indices, n_bins)
    ...
    # (node_id, depth, start_idx, end_idx)
    ...
    while len(stack) > 0:
        ...
        if n_left < n_right:
            hist_left = build_histograms(
                X_binned, y, indices[start_idx:split_idx], features_indices, n_bins
            )
            ...
            hist_right = build_histograms(
                X_binned, y, indices[split_idx:end_idx], features_indices, n_bins
            )
...
def predict_forest(X_binned, forest_structure):
    ...
    for i in prange(n_samples):
        ...
        predictions[i] = prediction_sum / n_trees
